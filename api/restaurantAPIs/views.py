...
#     permission_classes = [AllowAny]  # Хүн бүр нэмэх боломжтой
    
#     def post(self, request, resID):
#         """Рестораны зураг нэмэх/солих (хүн бүр нэмж болно)"""
#         try:
...
#                     "your_type": image_file.content_type
#                 },
#                 status=status.HTTP_400_BAD_REQUEST
#             )
        
#         # 3. Файлын хэмжээ шалгах (Optional: 5MB хүртэл)
...
#                     "max_size_mb": 5,
#                     "your_size_mb": round(image_file.size / (1024 * 1024), 2)
#                 },
#                 status=status.HTTP_400_BAD_REQUEST
#             )
        
#         # 4. Хуучин зургийг устгах (хэрэв байвал)
#         old_image_path = None
#         with connection.cursor() as c:
#             # Эхлээд хуучин зургийн зам авах
#             c.execute("""
#                 SELECT "image" FROM tbl_restaurant WHERE "resID" = %s
#             """, [resID])
...
#         # 5. Шинэ зураг хадгалах
#         import uuid
#         from django.conf import settings
#         import os
        
#         # Файлын өргөтгөл авч, жижиг үсэг болгох
#         file_extension = image_file.name.split('.')[-1].lower()
        
#         # UUID ашиглан өвөрмөц нэр үүсгэх
#         unique_filename = f"restaurant_{resID}_{uuid.uuid4().hex[:8]}.{file_extension}"
        
#         # Хадгалах зам бэлдэх
#         media_root = getattr(settings, 'MEDIA_ROOT', 'media')
#         upload_dir = os.path.join(media_root, 'restaurants')
        
#         # Директори байхгүй бол үүсгэх
#         if not os.path.exists(upload_dir):
#             os.makedirs(upload_dir)
...
#         # 7. Database-д шинэчлэх
#         relative_path = f"restaurants/{unique_filename}"
...
#             result = c.fetchone()
#             if not result:
#                 # Хэрэв ресторан олдохгүй бол хадгалсан файлаа устгах
...
#         # 8. Хуучин зургийг устгах (хэрэв байвал)
#         if old_image_path:
#             try:
#                 old_full_path = os.path.join(media_root, old_image_path)
#                 if os.path.exists(old_full_path):
#                     os.remove(old_full_path)
#             except Exception as e:
#                 # Хуучин зураг устгахад алдаа гарвал лог хадгалах
#                 print(f"Warning: Could not delete old image {old_image_path}: {str(e)}")
...
#     def delete(self, request, resID):
#         """Рестораны зураг устгах (хүн бүр устгаж болно)"""
#         try:
#             res_id_int = int(resID)
#         except ValueError:
#             return Response(
...
#         with connection.cursor() as c:
#             # 1. Ресторан байгаа эсэх, одоогийн зургийн мэдээлэл авах
#             c.execute("""
#                 SELECT "image", "resName" 
#                 FROM tbl_restaurant 
#                 WHERE "resID" = %s
#             """, [resID])
...
#             # 3. Физик файл устгах (хэрэв байвал)
#             if current_image:
#                 from django.conf import settings
#                 import os
                
#                 media_root = getattr(settings, 'MEDIA_ROOT', 'media')
#                 file_path = os.path.join(media_root, current_image)
...
#                 except Exception as e:
#                     file_deleted = False
#                     print(f"Warning: Could not delete image file {current_image}: {str(e)}")
#             else:
#                 file_deleted = False
...
#             "resID": resID,
#             "resName": res_name,
#             "image_was_deleted": file_deleted if 'file_deleted' in locals() else False
#         }
...
# class FoodImageUploadView(APIView):
    ...
    def post(self, request, foodID):
        ...
        unique_filename = f"food_{foodID}_{uuid.uuid4()}.{file_extension}"
        ...
        media_root = getattr(settings, 'MEDIA_ROOT', 'media')
        upload_dir = os.path.join(media_root, 'foods')
        ...
        # Database-д хадгалах
        relative_path = f"foods/{unique_filename}"
